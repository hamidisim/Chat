...
public class ChatServlet extends HttpServlet implements CometProcessor {
	...
	throws IOException, ServletException {
		...
		if (event.getEventType() == CometEvent.EventType.BEGIN) {
			log("Begin for session: " +  request.getRemoteAddr());
			event.setTimeout(900*1000*1000); /* timeout is 15 minutes */
			synchronized(connections) {
				connections.add(response);
			}
		} else if (event.getEventType() == CometEvent.EventType.ERROR) {
			log("Error for session: " +  request.getRemoteAddr());
			synchronized(connections) {
				connections.remove(response);
			}
			event.close();
		} else if (event.getEventType() == CometEvent.EventType.END) {
			log("End for session: " +  request.getRemoteAddr());
			synchronized(connections) {
				connections.remove(response);
			}
			event.close();
			...
			do {
				...
				if (n > 0) {
					...
					+ " for session: " + request.getRemoteAddr());
				} else if (n < 0) {
			...
			messageSender.send(request.getRemoteAddr(), message.toString());
		}
	}
	...
	public class MessageSender implements Runnable {
		...
		public void send(String user, String message) {
			synchronized (messages) {
				messages.add("MyIPAddress[" + user + "]: " + message);
				messages.notify();
			}
		}
